...
public class ArticlePage {
    ...
    public String waitForTitleContaining(String expectedText) {
        ...
        if (expectedText != null && !expectedText.isBlank()) {
            String safe = escapeUiText(expectedText);
            candidates.add(AppiumBy.androidUIAutomator("new UiSelector().textContains(\"" + safe + "\")"));
            candidates.add(AppiumBy.androidUIAutomator("new UiSelector().descriptionContains(\"" + safe + "\")"));
        }

        WebElement title = wait.until(d -> firstDisplayedElement(d, candidates));
    ...
    public boolean isTextVisible(String text) {
        String safe = escapeUiText(text);
        List<WebElement> elements = driver.findElements(
                AppiumBy.androidUIAutomator("new UiSelector().textContains(\"" + safe + "\")")
        );
    ...
    private WebElement firstDisplayedElement(org.openqa.selenium.WebDriver d, List<By> candidates) {
        for (By candidate : candidates) {
            List<WebElement> elements = d.findElements(candidate);
    ...
    private void waitForLoadingToFinish() {
        if (!driver.findElements(ARTICLE_PROGRESS).isEmpty()) {
